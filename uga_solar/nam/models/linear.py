...
class DataHelper:
    def __init__(self, start='20160901T0000', stop='20170831T1800', target_column=5):
        self.x = data.load_range(start, stop)
        self.y = data.load_targets(7)
...
class SingleLinear(torch.nn.Module):
    def __init__(self, n_features):
        super().__init__()
        # self.mean = torch.nn.Parameter(torch.zeros(1), requires_grad=False)
        # self.std = torch.nn.Parameter(torch.ones(1), requires_grad=False)
